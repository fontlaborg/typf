use typf_render_color::svg::{render_svg_glyph_with_palette, get_svg_document};
...
fn main() {
    ...
    let flipped = tiny_skia::Pixmap::from_vec(rgba, tiny_skia::IntSize::from_wh(w, h).unwrap()).unwrap();
    flipped.save_png("/tmp/svg_flipped.png").unwrap();
    println!("Saved /tmp/svg_flipped.png");
    
    // Also check colors
    let non_black = pixmap.data().chunks(4).filter(|p| p[3] > 0 && (p[0] > 10 || p[1] > 10 || p[2] > 10)).count();
