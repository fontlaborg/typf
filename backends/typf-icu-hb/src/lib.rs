...
impl TtfFaceEntry {
    fn new(data: Arc<FontDataEntry>) -> Result<Self> {
        ...
        let units_per_em = font_ref
            .head()
            .ok()
            .ok_or_else(|| TypfError::render("Failed to read head table"))?
            .units_per_em();

        let ascender_unscaled = font_ref
            .hhea()
            .ok()
            .ok_or_else(|| TypfError::render("Failed to read hhea table"))?
            .ascender()
            .to_i16();
...
impl Backend for HarfBuzzBackend {
    ...
    fn render(&self, shaped: &ShapingResult, options: &RenderOptions) -> Result<RenderOutput> {
        ...
        // Calculate baseline position (use cached ascender)
        let ascender = face_entry.ascender_unscaled as f32 * scale;
