...
impl TypfCoreBackend for HarfBuzzBackend {
    ...
    fn shape(&self, run: &TextRun, font: &Font) -> Result<ShapingResult> {
        ...
        use std::io::Write;
        if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/typf-skiahb-debug.log") {
            writeln!(f, "[SkiaHB] shape() - font.size={}, upem={}, scale={}",
    ...
    fn render(&self, shaped: &ShapingResult, options: &RenderOptions) -> Result<RenderOutput> {
        ...
        use std::io::Write;
        if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/typf-skiahb-debug.log") {
            writeln!(f, "[SkiaHB] render() - font.size={}, upem={}, scale={}, bbox={}x{}",
