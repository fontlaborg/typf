...
pub fn clear_all_caches() {
    if is_caching_enabled() {
        ...
        log::debug!("All global caches cleared.");
    } else {
        log::debug!("Caching is disabled, no caches to clear.");
    }
}
...
mod tests {
    ...
    #[test]
    fn test_clear_all_caches_clears_enabled_caches() {
        // Ensure caching is enabled for this test
        let _guard = scoped_caching_enabled(true);
        ...
        assert!(
            !manager.shaping_cache.is_empty(),
            "Shaping cache should not be empty before clear"
        );
        assert!(
            !manager.glyph_cache.is_empty(),
            "Glyph cache should not be empty before clear"
        );

        // Clear all caches
        clear_all_caches();

        // Ensure caches are empty after clearing
        manager.shaping_cache.sync(); // Force pending operations
        manager.glyph_cache.sync(); // Force pending operations
        assert!(
            manager.shaping_cache.is_empty(),
            "Shaping cache should be empty after clear"
        );
        assert!(
            manager.glyph_cache.is_empty(),
            "Glyph cache should be empty after clear"
        );
    }
}
