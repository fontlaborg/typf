...
- JSONL batch execution now rejects blank/whitespace-padded `job.id` values and rejects duplicate `job.id` values before processing
- JSONL job processing now rejects zero `rendering.width`/`rendering.height` values with explicit validation errors
- `typf batch` now trims and validates per-job `font` path values, rejecting blank inputs early
- `typf batch` now normalizes per-job `shaper`/`renderer` tokens (trim + lowercase + blank-default behavior) before dispatching to render
...
- JSONL stream validation now rejects duplicate `job.id` values across lines with explicit line-aware diagnostics
...
- `typf render` now honors `--face-index` by loading fonts with `TypfFontFace::from_file_index(...)`
- `typf render` font-load failures now include contextual `face_index=<n>` details
